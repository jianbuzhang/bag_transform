...
std::string path="/media/jinjian/JINJIAN18761511469/01_Work/MEMS_Dataset/07-13-old/";
std::string new_path="/media/jinjian/JINJIAN18761511469/01_Work/MEMS_Dataset/07-13-new/";
...
int main(int argc, char **argv)
{
    ...
    //double lidar2kit[6]={1.0,0.0,1.0,-0.08,0.12,0};
    double lidar2kit[6]={-0.5,0.1,1,-0.04,0.1,0};
    ...
    while((ptr=readdir(pDir))!=0)
    {
            ...
            topics.push_back(std::string("/usb_cam0/image_raw"));
            ...
            for(auto m:view)
            {
                ...
                if(topic == "/mems_sensing/lidar/concatenated/pointcloud")
                {
                        ...
                        o_bag.write("/mems_sensing/lidar/front/zvision_lidar_points",mems->header.stamp,t_rosCloud);
                }
                if(topic =="/usb_cam0/image_raw")
                {
                        sensor_msgs::Image::ConstPtr img = m.instantiate<sensor_msgs::Image>();
                        o_bag.write("/usb_cam0/image_raw",img->header.stamp,img);
                }

                
            }
            ROS_INFO("finished transform...");
    }
    }
    ROS_INFO("program over---");
   closedir(pDir);
ros::spin();
    return (0);
}
